...
The **flow** transfers the files to the **collection** which is colocated with the Compute **endpoint**, creates a tarfile from them, and transfers the tarfile to a user-provided destination **collection**. This flow behaves the same as its counterpart, the only difference being in that the source files to be tarballed and the destination collection are derived from the transfer data rather than the user input. This **flow** will fail if any source paths are the path '/' or contain the character '~'.
